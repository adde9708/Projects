...
def add_channel_point_reward(channel, reward, main_thread_id):
    ...
    if is_user_input_required:
        ...
        with channel_lock:
            ...
            channel_point_rewards_len = channel["channel_point_rewards_len"]
            channel_point_rewards_capacity = channel["channel_point_rewards_capacity"]

            if channel_point_rewards_len == channel_point_rewards_capacity:

                initial_capacity = 4

                new_capacity = (
                    initial_capacity
                    if channel_point_rewards_capacity == 0
                    else channel_point_rewards_capacity * 2
                )

                channel_point_rewards = channel["channel_point_rewards"]
                channel_point_rewards += [None] * (
                    new_capacity - channel_point_rewards_capacity
                )

                channel_point_rewards_capacity = new_capacity

            channel_point_rewards[channel_point_rewards_len] = reward
            channel_point_rewards_len += 1
    else:
...
def main_func(
    ...
    channel_point_reward = add_channel_point_reward(
        twitch_channel, reward, main_thread_id
    )
